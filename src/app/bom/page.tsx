...
export default function BomPage() {
  ...
  async function fetchAll() {
    const [p, r, pk] = await Promise.all([
      supabase.from('products').select('id, sku, name, size_oz').eq('is_active', true).order('sku'),
      supabase.from('raw_materials').select('id, item_no, name, unit').order('item_no'),
      supabase.from('packaging').select('id, item_no, name, type').order('item_no'),
    ])
    setProducts(p.data || [])
  ...
  async function fetchBomForProduct(productId: string) {
    const { data } = await supabase
      .from('bom')
      .select('*, raw_materials(item_no, name, unit), packaging(item_no, name, type)')
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ background: '#f8fafc' }}>
                          {['Type', 'Item #', 'Name', 'Qty per Unit', 'Unit', 'Notes', ''].map(h => (
